...
from app.dao.sql.base import SqlDaoBase
...
class GameDao(SqlDaoBase):
    ...
    async def create_game_players(self, game_id: uuid.UUID, players: list[GamePlayerCreate]) -> list[Game]:
        query = """
        INSERT INTO game_players (game_id, player_id, side, type) 
        VALUES ($1, $2, $3, $4)
        """
        data = [
            (game_id, p.player_id, p.side, p.type) 
            for p in players
        ]
