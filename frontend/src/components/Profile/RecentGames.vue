...
import { getRecentGames, type GameDetails } from '@/api/player'
...
const getRowClass = (game: GameDetails) => {
    ...
    <div class="rounded-lg border border-gray-600 shadow-lg overflow-hidden">
      <table class="w-full text-left border-separate border-spacing-0">
        <thead class="bg-gray-600 text-gray-300 uppercase text-xs font-semibold">
          <tr>
            <th class="px-4 py-3">Player X</th>
            <th class="px-4 py-3">Player O</th>
        ...
        <tbody class="divide-y divide-gray-700">
          <tr v-if="games.length === 0">
            <td colspan="4" class="px-4 py-10 text-center text-gray-500">Нет недавних игр</td>
          </tr>

          <tr
            v-for="game in gamesWithPlayers"
            :key="game.id"
            :class="['transition-colors duration-200', getRowClass(game)]"
          >
            ...
            <td class="px-4 py-3 text-right text-white tabular-nums">{{ game.duration }}с</td>
          </tr>
        </tbody>
      </table>
    </div>
